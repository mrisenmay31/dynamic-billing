...
| B-05 | TC-8 | P1 | Entries deleted/voided in QB Time persist in ClockToBill after re-sync | Re-sync mirrors QB Time (orphans removed) | Stale entries remained → over-billing | Sync, delete an entry in QB Time, re-sync | FIXED + MERGED + **DEPLOYED** (`494e1bd`) — empty-fetch + sent-invoice guards; **live delete-test = NEXT TASK** | yes (Lane B) |
...
| B-07 | TC-17 | P0 | Non-billable per-entry time billed on MIXED clients; `is_billable: true` hardcoded at sync (`sync-timesheets/route.ts:167`), `engine.ts` filter is a no-op. June CTA over-bill **$281.25** (Greenleaf, Ironclad, Mesa Verde). Path B (whole-customer `exclude_from_billing`) already shipped but cannot drop individual "No" entries. | No non-billable time produces an invoice line | Non-billable analysis time invoiced at $125/hr | Sync CTA June, Generate Drafts, compare to `customfields.6490032` split | **Path B + Path A FIXED + MERGED + DEPLOYED (`494e1bd`); verified live (95 Yes / 4 No, 0 mismatches at time_entries level). Draft-level confirmation pending post-sweep regenerate.** | yes (Lane B, Path A) |
